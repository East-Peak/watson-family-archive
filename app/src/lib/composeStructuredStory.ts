...
interface BiographyData {
  occupations?: string[];
  timelineHighlights?: Array<{ event: string; year: number; location?: string }>;
}
...
export function composeStructuredStory(
  ...
  if (family?.father && family?.mother) {
    ...
    const childType = person.sex === 'M' ? 'Son'
      : person.sex === 'F' ? 'Daughter'
      : 'Child';
    originParts.push(`${childType} of ${fatherFirst} and ${motherFirst}.`);
  } else if (family?.father) {
    originParts.push(`${possessive} father was ${family.father.name.split(' ')[0]}.`);
  } else if (family?.mother) {
    originParts.push(`${possessive} mother was ${family.mother.name.split(' ')[0]}.`);
  }
  ...
  if (biography?.occupations && biography.occupations.length > 0) {
    const validOccs = biography.occupations.filter(o =>
      o && o.length > 2 && o.length < 60 &&
      !o.includes('###') && !o.includes('**') && !o.includes('GEDCOM')
    );
    if (validOccs.length === 1) {
      ...
      const rest = validOccs.slice(0, -1).map(o => o.toLowerCase()).join(', ');
      paragraphs.push(`${pronoun} worked as ${rest} and ${last}.`);
    }
  }

  // ── Census appearances ──
  const censusEvents = biography?.timelineHighlights?.filter(h =>
    h.event.toLowerCase().includes('census')
  ) || [];
  if (censusEvents.length > 0) {
    const years = censusEvents.map(e => e.year).filter(Boolean).sort();
    const locations = [...new Set(censusEvents.map(e => e.location).filter(Boolean))];

    if (years.length > 0) {
      const yearStr = years.length <= 3
        ? years.join(', ').replace(/,\s([^,]+)$/, ' and $1')
        : `${years[0]} through ${years[years.length - 1]}`;
      const locStr = locations.length > 0 ? ` in ${locations[0]}` : '';
      paragraphs.push(`Appears in ${years.length > 1 ? '' : 'the '}${yearStr} census record${years.length > 1 ? 's' : ''}${locStr}.`);
    }
  }
  ...
  if (family?.children && family.children.length > 0) {
    ...
    familyParts.push(`Together they had ${count} known ${count === 1 ? 'child' : 'children'}.`);
  }
  ...
  // ── Death paragraph ──
  if (!person.isLiving && (person.deathDate || person.deathYear || person.deathPlace)) {
