...
import type { ArcColorMode, GlobeData, GlobeViewMode, GlobeViewState } from './types';
...
const VIEW_MODES: {
  ...
  },
  {
    ...
    label: 'Generation',
    requiresViewer: true,
    description: 'The same places, recolored by how many generations back each ancestor sits from the viewer.',
  },
  {
    mode: 'origins',
    label: 'Origins',
...
export default function ControlDock({
  ...
  const showMyLinesOption = hasViewer || viewerAncestryLoading || Boolean(viewerAncestryError) || hasLoadedViewerAncestry;
  const myLinesDisabled = viewerAncestryLoading || Boolean(viewerAncestryError) || !hasLoadedViewerAncestry;
  ...
  const updateScrollAffordance = useCallback(() => {
    ...
    const remainingScroll = scrollBody.scrollHeight - scrollBody.clientHeight - scrollBody.scrollTop;
  ...
  if (collapsed) {
    return (
      <div className="absolute top-4 right-4 z-10">
        <button
          onClick={() => { setCollapsed(false); onCloseSidebar?.(); }}
  ...
  return (
    <div className="absolute top-4 left-4 right-4 bottom-24 z-10 flex w-auto flex-col overflow-hidden rounded-2xl border border-white/10 bg-black/60 shadow-2xl backdrop-blur-md sm:left-auto sm:w-64">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 flex-shrink-0">
        <span className="text-sm font-semibold text-white tracking-wide">Controls</span>
        <button
      ...
      <div className="relative flex-1 min-h-0">
        ...
        <div className="px-4 py-3 border-b border-white/5">
          <div className="flex items-center justify-between mb-1.5">
            <div className="text-xs text-white/50 uppercase tracking-wider font-semibold">Family Tree</div>
            <button
              onClick={handleShare}
          ...
          <div className="text-white/70 text-sm">
            ...
            {(dataQuality.approximate > 0 || dataQuality.missing > 0) && (
              <span className="text-white/40">
                {' '}({dataQuality.approximate > 0 && `${dataQuality.approximate} approx`}{dataQuality.approximate > 0 && dataQuality.missing > 0 && ', '}{dataQuality.missing > 0 && `${dataQuality.missing} missing`})
              </span>
            )}
          </div>
          <div className="text-white/70 text-sm">{arcCount} migration paths</div>
        </div>

        {/* Section 2: Branch filter */}
        <div className="px-4 py-3 border-b border-white/5">
          <label className="text-xs text-white/50 uppercase tracking-wider font-semibold">Filter by Family</label>
          <select
          ...
          {viewState.branch === 'my-lines' && viewerAncestryLoading && (
            <p className="mt-2 text-xs text-white/45">Loading your ancestor lines...</p>
          )}
        ...
        <div className="px-4 py-3 border-b border-white/5">
          <div className="text-xs text-white/50 uppercase tracking-wider font-semibold mb-2">View Mode</div>
            ...
            {VIEW_MODES.map(({ mode, label, requiresViewer }) => {
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  title={isDisabled ? 'Set "Who am I?" to enable this mode' : `Switch to ${label} view`}
          ...
          <div className="mt-2 rounded-xl border border-white/8 bg-white/[0.04] px-3 py-2">
            ...
            {!hasViewer && (
              <p className="mt-1 text-[11px] leading-5 text-white/30">
                Set your viewer profile to unlock generation and origins views.
              </p>
            )}
          </div>
        </div>
        ...
        {/* Section 6: Layers */}
        <div className="px-4 py-3 border-b border-white/5">
          <div className="text-xs text-white/50 uppercase tracking-wider font-semibold mb-2">Layers</div>
          <div className="space-y-2">
            ...
            >
              ...
              >
                <div
                  className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow transition-transform ${
                    viewState.showLabels ? 'translate-x-4' : 'translate-x-0.5'
                  }`}
                />
              </div>
            </button>
            ...
            >
              ...
              <div
                className={`w-8 h-4.5 rounded-full relative transition-colors ${
                  viewState.showApproximate ? 'bg-indigo-500' : 'bg-white/20'
                }`}
              >
                <div
                  className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow transition-transform ${
                    viewState.showApproximate ? 'translate-x-4' : 'translate-x-0.5'
                  }`}
                />
              </div>
            </button>
        ...
        <div className="px-4 py-3 border-b border-white/5">
          <div className="text-xs text-white/50 uppercase tracking-wider font-semibold mb-2">Locations by Region</div>
          <div className="space-y-1">
            {regionOptions.map((region) => {
                const color = REGION_COLORS[region] || REGION_COLORS.default;
                const isExplicitlySelected = viewState.regions.includes(region);
            ...
            {!canScrollUp && (
              <div className="absolute inset-x-0 bottom-2 flex justify-center px-4">
                ...
                >
                  ...
                  <svg className="h-3.5 w-3.5 animate-[bounce_1.6s_ease-in-out_infinite]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
