...
export default function PersonResearch({
  ...
  const hasResearchData = biography?.verifiedFacts?.length || biography?.keySources?.length ||
    wikitreeId || findagraveId || familysearchTreeId || biography?.externalLinks?.length || biographyMarkdown || sources.length > 0;
  ...
  return (
    <section className="bg-white/80 backdrop-blur-md rounded-2xl border border-white p-6 shadow-xl hover:shadow-2xl transition-shadow relative overflow-hidden group/container">
      ...
      <h2 className="text-sm font-bold text-shield uppercase tracking-widest mb-6 mt-2 relative z-10">Research</h2>
      ...
      <div className="space-y-4 relative z-10">
            ...
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {Object.entries(byType)
                  ...
                  .map(([type, items]) => (
                    <span key={type} className="inline-flex items-center gap-1 px-2 py-1 bg-white border border-gray-200 rounded-lg text-xs text-gray-700">
                  ...
                  <div key={idx} className="flex items-center gap-2 text-sm">
                    {s.tier && (
                      <span className={`w-5 h-5 rounded text-xs font-bold flex items-center justify-center shrink-0 ${
                        s.tier === 'A' ? 'bg-green-100 text-green-700' :
                        s.tier === 'B' ? 'bg-blue-100 text-blue-700' :
                        s.tier === 'C' ? 'bg-amber-100 text-amber-700' :
                    ...
                    {s.url ? (
                      <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:text-indigo-800 truncate">
                        {s.collection || s.recordType}
                      </a>
                    ) : (
                      <span className="text-gray-600 truncate">{s.collection || s.recordType}</span>
                    )}
                    {s.year && <span className="text-gray-400 text-xs ml-auto shrink-0">{s.year}</span>}
                  </div>
                ))}
                {sources.length > 8 && (
                  <p className="text-xs text-gray-400 mt-1">+{sources.length - 8} more</p>
                )}
              </div>
            </div>
          );
          ...
          <details className="group">
            <summary className="flex items-center justify-between cursor-pointer p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-900 font-medium">{biography.verifiedFacts.length} Verified Facts</span>
              </div>
        ...
        {biography?.keySources && biography.keySources.length > 0 && (
          <details className="group">
            <summary className="flex items-center justify-between cursor-pointer p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
              <div className="flex items-center gap-3">
                <svg className="w-5 h-5 text-shield" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
                <span className="text-gray-900 font-medium">{biography.keySources.length} Sources</span>
              </div>
              <svg className="w-5 h-5 text-gray-400 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            ...
            <div className="mt-2 p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-2">
              {biography.keySources.map((source, idx) => (
                <p key={idx} className="text-gray-600 text-sm">{idx + 1}. {source}</p>
              ))}
            </div>
          </details>
        )}
        ...
        {/* External links */}
        {(wikitreeId || findagraveId || familysearchTreeId || biography?.externalLinks?.length) && (
          <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
        ...
        {biographyMarkdown && (
          <button
            onClick={() => {
              ...
              const blob = new Blob([biographyMarkdown], { type: 'text/markdown' });
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-gray-900 font-medium">Full Research File</span>
                <p className="text-xs text-gray-500">Download complete research notes</p>
              </div>
            </div>
            <svg className="w-5 h-5 text-shield group-hover:translate-y-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
