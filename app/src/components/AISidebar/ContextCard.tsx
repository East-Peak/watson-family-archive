...
export default function ContextCard({ personId, personName, onQuickAction }: ContextCardProps) {
  ...
  return (
    <div className="border-b border-amber-200/40 bg-amber-50/30 px-4 py-3">
      <h3 className="font-semibold text-gray-900 text-sm">{personName || 'Loading...'}</h3>
      {loading ? (
        ...
        <p className="text-xs text-gray-500 mt-1">
          {summary.birthYear && summary.deathYear ? `${summary.birthYear}–${summary.deathYear}` : summary.birthYear ? `b. ${summary.birthYear}` : ''}
          {summary.birthPlace && <span className="ml-1">{summary.birthYear || summary.deathYear ? ' · ' : ''}{summary.birthPlace}</span>}
        </p>
      ) : null}
      <div className="flex gap-2 mt-2">
        <button type="button" onClick={() => onQuickAction(`What are the research gaps for ${firstName}?`)}
        ...
        <button type="button" onClick={() => onQuickAction(`Show me all records for ${firstName}`)}
          className="px-2 py-1 text-xs bg-white border border-amber-200/60 text-gray-600 rounded hover:bg-amber-50 transition-colors">
          Show records
        </button>
      </div>
