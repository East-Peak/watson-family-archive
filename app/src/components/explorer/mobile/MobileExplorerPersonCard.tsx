...
function MobileExplorerPersonCard({ person }: { person: ExplorerPerson }) {
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="text-base font-semibold leading-tight text-shield">
            {person.fullName}
          </h3>
          {person.maidenName && (
        ...
        <span
          className={`shrink-0 rounded-full border px-2 py-1 text-xs font-semibold ${statusColor}`}
        >
      ...
      <dl className="mt-3 space-y-2 text-sm text-slate-600">
        <div>
          <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-shield/45">
            Birth
          </dt>
          <dd className="mt-1">
            {formatLifeEvent(person.birthYear, person.birthPlace)}
          </dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-[0.18em] text-shield/45">
          ...
          <dd className="mt-1">
            {formatLifeEvent(person.deathYear, person.deathPlace)}
          </dd>
        </div>
      </dl>
      ...
      <div className="mt-4 flex items-center gap-3">
        <div className="flex-1">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Completeness {person.completenessScore}%
          </p>
        </div>
...
export default memo(MobileExplorerPersonCard);
