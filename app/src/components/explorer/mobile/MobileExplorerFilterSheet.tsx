...
const SECTION_CLASS = 'space-y-3 rounded-2xl border border-shield/10 bg-shield/[0.03] p-4';
const LABEL_CLASS = 'block text-xs font-semibold uppercase tracking-[0.18em] text-shield/45';
const INPUT_CLASS = 'mt-2 min-h-11 w-full rounded-2xl border border-shield/15 bg-white px-3 py-2 text-sm text-slate-700';
...
export default function MobileExplorerFilterSheet({
  ...
  const renderSortControls = (sortField: string, sortDirection: SortDirection) => (
      ...
      <label className="block">
        ...
        <select
          ...
          onChange={(event) => {
            ...
            if (isRecordsMode) {
              onStateChange({ recordSortField: nextField as RecordSortField, recordSortDirection: 'asc' });
            } else {
              onStateChange({ sortField: nextField as SortField, sortDirection: 'asc' });
            }
          }}
          className={INPUT_CLASS}
        >
          {(isRecordsMode ? RECORD_SORT_OPTIONS : PEOPLE_SORT_OPTIONS).map((option) => (
  ...
  return (
    <BottomSheet open={open} onClose={handleClose} eyebrow="Explorer" title="Filters">
      <div className="space-y-5">
        {isRecordsMode ? (
          <>
                    ...
                    {recordsFilterOptions.types.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => onStateChange({ recordTypes: toggleListItem(viewState.recordTypes, type) })}
                  ...
                  {['A', 'B', 'C', 'D', 'E'].map((tier) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        aria-pressed={isActive}
                        onClick={() => onStateChange({ tiers: toggleListItem(viewState.tiers, tier) })}
                        className={`min-h-10 rounded-full border px-3 py-2 text-sm font-medium transition-colors ${
                          isActive
                            ? 'border-shield bg-shield text-white'
                            : 'border-shield/15 bg-white text-shield hover:bg-shield/5'
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      onStateChange({ yearMin: Number.isNaN(value) ? 0 : value });
                    }}
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      const value = event.target.value === '' ? 9999 : Number.parseInt(event.target.value, 10);
                      onStateChange({ yearMax: Number.isNaN(value) ? 9999 : value });
                    }}
            ...
            {renderSortControls(viewState.recordSortField, viewState.recordSortDirection)}
          </>
        ) : (
          <>
            <section className={SECTION_CLASS}>
                    ...
                    {filterOptions.centuries.map((century) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          aria-pressed={isActive}
                          onClick={() => onStateChange({ centuries: toggleListItem(viewState.centuries, century) })}
                          className={`min-h-10 rounded-full border px-3 py-2 text-sm font-medium transition-colors ${
                            isActive
                              ? 'border-shield bg-shield text-white'
                              : 'border-shield/15 bg-white text-shield hover:bg-shield/5'
                    ...
                    {filterOptions.countries.map((country) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          aria-pressed={isActive}
                          onClick={() => onStateChange({ countries: toggleListItem(viewState.countries, country) })}
                          className={`min-h-10 rounded-full border px-3 py-2 text-sm font-medium transition-colors ${
                            isActive
                              ? 'border-shield bg-shield text-white'
                              : 'border-shield/15 bg-white text-shield hover:bg-shield/5'
                    ...
                    {filterOptions.statuses.map((status) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          aria-pressed={isActive}
                          onClick={() => onStateChange({ statuses: toggleListItem(viewState.statuses, status) })}
                          className={`min-h-10 rounded-full border px-3 py-2 text-sm font-medium transition-colors ${
                            isActive
                              ? 'border-shield bg-shield text-white'
                              : 'border-shield/15 bg-white text-shield hover:bg-shield/5'
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <select
                    aria-label="Sex"
                    value={viewState.sex}
                    onChange={(event) => onStateChange({ sex: event.target.value })}
                    className={INPUT_CLASS}
                  >
                ...
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(event) => onStateChange({ hasSources: event.target.value })}
                    className={INPUT_CLASS}
                  >
                    <option value="">All</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      onStateChange({ completenessMin: Number.isNaN(value) ? 0 : value });
                    }}
                    className={INPUT_CLASS}
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      const value = Number.parseInt(event.target.value, 10);
                      onStateChange({ completenessMax: Number.isNaN(value) ? 100 : value });
                    }}
                    className={INPUT_CLASS}
                  />
                </label>
              ...
              <label className="block">
                ...
                <select
                  ...
                  onChange={(event) => onStateChange({ validation: event.target.value })}
                  className={INPUT_CLASS}
                >
                  <option value="">All</option>
