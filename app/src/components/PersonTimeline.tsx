...
export default function PersonTimeline({
  ...
  return (
    <section className="bg-white/80 backdrop-blur-md rounded-2xl border border-white p-8 shadow-xl hover:shadow-2xl transition-shadow relative overflow-hidden group">
      ...
      <h2 className="text-sm font-bold text-shield uppercase tracking-widest mb-8 relative z-10">
        Timeline
      </h2>
      ...
      <div className="relative z-10">
        ...
        <div className="space-y-6">
          {events.map((event, idx) => (
            <div
              key={idx}
              className={`relative flex gap-4 pl-10 ${event.isOutsideLifespan ? 'opacity-70' : ''}`}
            >
              {/* Dot */}
              <div
              ...
              <div className="flex-1 pb-2">
                <div className="flex items-baseline gap-3 flex-wrap">
                  <span
                    className={`text-lg font-semibold ${event.type === 'birth' ? 'text-emerald-600' : event.type === 'death' ? 'text-gray-500' : 'text-gray-900'}`}
                  >
                    {event.year || '?'}
                  </span>
                  <span
                    className={`font-medium ${event.type === 'birth' ? 'text-emerald-600' : event.type === 'death' ? 'text-gray-500' : event.isOutsideLifespan ? 'text-gray-400' : 'text-gray-700'}`}
                  >
                    {event.title}
                  </span>
                  {event.type === 'family' && (
