...
export async function handleRelationshipQuery(
  ...
  switch (relationship) {
    ...
    case 'mother': {
      ...
      const results = await executeQuery<{ id: string; name: string; birthYear: number | null; deathYear: number | null; birthPlace: string | null }>(
      ...
      const p = results[0];
      const dates = p.birthYear && p.deathYear ? ` (${p.birthYear}–${p.deathYear})` : p.birthYear ? ` (b. ${p.birthYear})` : '';
    ...
    case 'parents': {
      const results = await executeQuery<{ id: string; name: string; sex: string; birthYear: number | null; deathYear: number | null }>(
        `MATCH (t:Tree {id: $treeId})-[:CONTAINS]->(child:Person {id: $anchorId})
         MATCH (child)-[:CHILD_OF]->(parent:Person)
      ...
      if (results.length === 0) {
        return { response: `I don't have parents recorded for you in the family tree data.`, people: [] };
      }
      const lines = results.map(p => {
        const role = p.sex === 'M' ? 'Father' : p.sex === 'F' ? 'Mother' : 'Parent';
        const dates = p.birthYear && p.deathYear ? ` (${p.birthYear}–${p.deathYear})` : '';
    ...
    case 'spouse': {
      const results = await executeQuery<{ id: string; name: string; birthYear: number | null; deathYear: number | null }>(
      ...
      if (results.length === 0) {
        return { response: `I don't have a spouse recorded for you in the family tree data.`, people: [] };
      }
      if (results.length === 1) {
        const s = results[0];
        const dates = s.birthYear && s.deathYear ? ` (${s.birthYear}–${s.deathYear})` : '';
        return {
    ...
    case 'children': {
      ...
      if (results.length === 0) {
        return { response: `I don't have children recorded for you in the family tree data.`, people: [] };
      }
      const lines = results.map(c => {
        const dates = c.birthYear && c.deathYear ? ` (${c.birthYear}–${c.deathYear})` : c.birthYear ? ` (b. ${c.birthYear})` : '';
...
function extractRelationship(plan: QueryPlan): string | null {
  ...
  const relTerms = ['father', 'mother', 'parents', 'spouse', 'husband', 'wife', 'children', 'child', 'son', 'daughter'];
  for (const constraint of plan.constraints) {
    const lower = constraint.toLowerCase();
    if (relTerms.includes(lower)) {
      // Normalize
      if (lower === 'husband' || lower === 'wife') return 'spouse';
      if (lower === 'son' || lower === 'daughter' || lower === 'child') return 'children';
      return lower;
    }
  }
  // Also check the retrievalSpec for relationship terms
  if (plan.retrievalSpec) {
    for (const filter of [...plan.retrievalSpec.hardFilters, ...plan.retrievalSpec.softBoosts]) {
