...
export async function GET(request: NextRequest) {
  try {
    ...
    const rawLimit = searchParams.get('limit')
      ...
      : 8;
    const rawMinCount = searchParams.get('minCount')
      ? parseInt(searchParams.get('minCount')!, 10)
      : 2;
    const limit = Number.isFinite(rawLimit) && rawLimit >= 0 ? rawLimit : 8;
    const minCount =
      Number.isFinite(rawMinCount) && rawMinCount >= 0 ? rawMinCount : 2;
    ...
    if (personId) {
      ...
      for (const row of results) {
        ...
        line.count++;
        if (row.birthYear) {
          if (!line.earliest || row.birthYear < line.earliest)
            line.earliest = row.birthYear;
          if (!line.latest || row.birthYear > line.latest)
            line.latest = row.birthYear;
        }
