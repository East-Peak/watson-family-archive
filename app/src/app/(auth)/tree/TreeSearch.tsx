  ...
  return (
    <div ref={containerRef} className="relative">
      <div className="flex items-center gap-1.5 bg-shield/5 border border-shield/15 rounded-full px-3 py-1.5">
        <svg
          className="w-3.5 h-3.5 text-shield/40"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        <input
          ...
          aria-activedescendant={
            activeIndex >= 0 ? `tree-search-result-${activeIndex}` : undefined
          }
      ...
      {open && (
        <div
          className="absolute top-full mt-1 left-0 right-0 min-w-[220px] bg-white border border-shield/15 rounded-xl shadow-xl overflow-hidden z-50"
          role="listbox"
          id="tree-search-results-listbox"
        >
          {results.map((person, index) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 transition-colors ${
                index === activeIndex ? 'bg-shield/10' : 'hover:bg-shield/5'
              }`}
            >
              <div className="text-sm font-medium text-shield">
                {person.name}
              </div>
              {person.birthYear && (
                <div className="text-xs text-shield/50">
                  b. {person.birthYear}
                </div>
              )}
            </button>
