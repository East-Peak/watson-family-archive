...
import { EditorialCard } from '@/components/ActivityFeed';
...
export default function Home() {
  ...
  const [spotlightAncestor, setSpotlightAncestor] = useState<SpotlightAncestor | null>(null);
  const [loaded, setLoaded] = useState(false);
  const [activityEntries, setActivityEntries] = useState<FeedEntry[]>([]);
  const [collections, setCollections] = useState<CollectionCard[]>([]);
  const { openSearch } = useChat();
  const { me, authIdentity } = useMe();
  const { ancestorIds, ancestorSurnames, ancestorCountries } = useViewerAncestry();
  ...
  const nextAncestor = useCallback(() => {
    if (allAncestors.length > 0) {
      const current = allAncestors.findIndex(a => a.id === spotlightAncestor?.id);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const homeUrl = me?.id ? `/api/home?viewerId=${encodeURIComponent(me.id)}` : '/api/home';
        const [homeRes, photosRes, discoveriesRes] = await Promise.all([
          fetch(homeUrl),
          fetch('/data/photos.json').catch(() => ({ json: () => ({ photos: [], byPerson: {} }) })),
          fetch('/api/activity?limit=6').catch(() => null),
        ]);
        ...
        const photosData: { photos: Photo[]; byPerson: Record<string, number[]> } = await photosRes.json();

        setViewerStats(homeData.viewerStats);
        setTreeStats(homeData.treeStats);

        // Enhance spotlight data with photos
        const spotlightWithPhotos: SpotlightAncestor[] = (homeData.spotlight || []).map(
          (person: SpotlightAncestor & { id: string }) => {
            const photoIndices = photosData.byPerson[person.id] || [];
            const personPhotos = photoIndices
              .map((idx: number) => photosData.photos?.[idx])
              .filter((p: Photo | undefined): p is Photo => p !== undefined && p.isPortrait);
        ...
        const storiesWithLinks: Story[] = (homeData.stories || []).map((story: Story) => ({
          ...story,
        }));

        setAllStories(storiesWithLinks);
        setStories(storiesWithLinks.sort(() => Math.random() - 0.5).slice(0, 3));

        // Load activity
        if (discoveriesRes?.ok) {
          const activityData = await discoveriesRes.json();
          setActivityEntries(activityData.entries || []);
        }
  ...
  const generations = activeStats?.latestBirth != null && activeStats?.earliestBirth != null
  ...
  const sortedCollections = useMemo(() => {
    ...
    if (ancestorCountries.size === 0 && ancestorSurnames.size === 0) return collections;
    ...
    <main className="min-h-full bg-parchment">
      ...
      <section className="relative bg-hero-gradient overflow-hidden pt-16 pb-28 px-6 border-b border-shield shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          {treeStats && (
            <MotionWrapper delay={0.2} className="grid grid-cols-2 md:grid-cols-6 gap-4 max-w-6xl mx-auto mb-16">
              <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                <div className="text-4xl md:text-5xl font-serif font-bold text-white text-glow mb-2">
                  <CountUp value={viewerStats?.totalIndividuals ?? treeStats.totalIndividuals} />
                </div>
                <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">{viewerStats ? 'Direct Ancestors' : 'Individuals'}</div>
              </div>
              <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                <div className="text-4xl md:text-5xl font-serif font-bold text-white text-glow mb-2">
                  <CountUp value={generations} />
                </div>
                <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">Generations</div>
              </div>
              <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                <div className="text-4xl md:text-5xl font-serif font-bold text-white text-glow mb-2">
                  <CountUp value={treeStats.totalPlaces} />
                </div>
                <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">Places</div>
              </div>
              <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                <div className="text-4xl md:text-5xl font-serif font-bold text-white text-glow mb-2">
                  <CountUp value={treeStats.totalCountries} />
                </div>
                <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">Countries</div>
              </div>
              <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                <div className="text-4xl md:text-5xl font-serif font-bold text-white text-glow mb-2">
                  {(viewerStats ?? treeStats).earliestBirth}
                </div>
                <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">Earliest Record</div>
              </div>
              {treeStats.totalRecords > 0 && (
                <div className="glass-panel rounded-2xl p-6 text-center transition-all duration-300 hover:bg-white/20 hover:-translate-y-1">
                  ...
                  <div className="text-white/70 text-xs font-semibold uppercase tracking-widest">Source Records</div>
                </div>
              )}
            </MotionWrapper>
          )}
          ...
          <MotionWrapper delay={0.4} className="flex flex-wrap justify-center gap-4">
            <Link
              href="/globe"
            ...
            >
              ...
              Search or Ask AI
              <kbd className="ml-2 px-2 py-0.5 text-xs bg-white/20 rounded text-white/80 border border-white/10">⌘K</kbd>
            </button>
          </MotionWrapper>
        </div>
      </section>
          ...
          <MotionWrapper className="max-w-4xl mx-auto">
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/40 group">
              <div className="bg-shield/5 px-6 py-4 flex items-center justify-between border-b border-shield/10">
                <h3 className="text-amber-400 text-sm font-medium uppercase tracking-wide">Ancestor Spotlight</h3>
                <button
                ...
                <div className="p-8 md:p-10 flex flex-col md:flex-row gap-8 items-center md:items-start transition-all duration-300">
                  ...
                  <div className="w-36 h-48 md:w-48 md:h-64 rounded-xl overflow-hidden bg-parchment flex-shrink-0 relative shadow-inner shadow-trunk/20 ring-1 ring-shield/10">
                    {spotlightAncestor.photoPath ? (
                      ...
                      <Image
                        src={spotlightAncestor.sex === 'F' ? '/images/silhouette-female.png' : '/images/silhouette-male.png'}
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h4 className="font-serif text-3xl md:text-4xl text-shield mb-2 tracking-tight">{spotlightAncestor.name}</h4>
                    <p className="text-shield/60 font-serif text-lg mb-4 italic">{spotlightAncestor.years}</p>
                    <p className="text-gray-800 font-medium text-lg leading-relaxed mb-6 border-l-4 border-oak/30 pl-4">{spotlightAncestor.tagline}</p>
                    {spotlightAncestor.birthPlace && (
                      <p className="text-gray-600 text-sm">
                        <span className="text-gray-400">Born:</span> {spotlightAncestor.birthPlace}
                      </p>
                    )}
                    {spotlightAncestor.deathPlace && (
                      <p className="text-gray-600 text-sm">
                        <span className="text-gray-400">Died:</span> {spotlightAncestor.deathPlace}
                      </p>
                    )}
        ...
        <section className="py-20 px-6 bg-vignette">
          <MotionWrapper className="max-w-6xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <div>
                <Link href="/collections" className="hover:underline"><h3 className="font-serif text-2xl text-shield">Discoveries</h3></Link>
                <p className="text-gray-600 text-sm">Interesting findings from the family archives</p>
              </div>
              <button
              ...
              {stories.map((story, idx) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-shield/70 text-xs font-medium uppercase tracking-wide">{story.category}</span>
                  </div>
                  <h4 className="font-serif text-xl text-gray-900 mb-2">{story.title}</h4>
                  <p className="text-gray-600 text-sm mb-4">{story.description}</p>
                  {(story.collectionType || story.personId) && (
                    <Link
                      href={story.collectionType ? `/collection/${story.collectionType}` : `/person/${story.personId}`}
      ...
      <section className="py-12 px-6 bg-cream border-b border-oak/10">
        <MotionWrapper className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              <Link href="/activity" className="hover:underline"><h3 className="font-serif text-2xl text-shield">Recent Activity</h3></Link>
              <p className="text-gray-500 text-sm">Latest updates to the family tree</p>
            </div>
          </div>
          {activityEntries.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {activityEntries.slice(0, 6).map((entry) => {
                if (entry.category === 'editorial') {
                  return (
                    <div key={entry.id} className="md:col-span-2 lg:col-span-3">
                      <EditorialCard entry={entry} />
                    </div>
                  );
                }

                const personSlug = entry.people?.[0];
                const inYourLine = personSlug && me && ancestorIds.has(personSlug);
                ...
                return (
                  ...
                  >
                    {personSlug ? (
                      <Link href={`/person/${personSlug}`} className="block">
                        <div className="flex items-start justify-between gap-2">
                          <div className="font-medium text-gray-900 text-sm mb-1">{entry.headline}</div>
                          {inYourLine && (
                            <span className="text-[10px] font-medium px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded-full whitespace-nowrap flex-shrink-0">
                              Your line
                            </span>
                          )}
                        </div>
                      ...
                      <div className="font-medium text-gray-900 text-sm">{entry.headline}</div>
                    )}
                  </div>
                );
              })}
          ...
          <div className="mt-6 text-center">
            <Link href="/activity" className="text-sm text-shield hover:underline">
      ...
      <section className="py-20 px-6 bg-parchment border-b border-oak/10">
        <MotionWrapper className="max-w-6xl mx-auto">
          ...
          <p className="text-gray-500 text-sm mb-6">
            {hasViewerPerson(me) ? 'Branches of your ancestry' : 'Major surname branches in the tree'}
          </p>
          {lines.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {lines.map((line) => (
                ...
                >
                  <div className="font-serif text-2xl text-shield mb-2 group-hover:text-shield/80 transition-colors">{line.surname}</div>
                  <div className="text-sm text-gray-600">{line.count} ancestors</div>
                  {line.earliest && line.latest && (
                    <div className="text-xs text-gray-400 mt-1">{line.earliest}–{line.latest}</div>
                  )}
                </Link>
              ))}
            </div>
          ) : (
            <p className="text-gray-400 text-sm py-4">No family lines found yet.</p>
          )}
          <div className="mt-6 text-center">
      ...
      <section className="py-12 px-6 bg-cream border-b border-oak/10">
        <MotionWrapper className="max-w-6xl mx-auto">
          <Link href="/collections" className="hover:underline">
            <h3 className="font-serif text-2xl text-shield mb-6">Collections</h3>
          </Link>
          {sortedCollections.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {sortedCollections.slice(0, 12).map((c) => {
                const isViewerRelevant = me && (
                  (c.type.match(/^(.+)-heritage$/) && ancestorCountries.has(c.type.replace('-heritage', ''))) ||
                  (c.type.match(/^(.+)-immigration$/) && ancestorCountries.has(c.type.replace('-immigration', ''))) ||
                  (c.type.match(/^surname-(.+)$/) && ancestorSurnames.has(c.type.replace('surname-', '')))
                );
                return (
                  <Link
                    key={c.type}
            ...
            <p className="text-gray-400 text-sm py-4">No collections available.</p>
          )}
          <div className="mt-6 text-center">
            <Link href="/collections" className="text-sm text-shield hover:underline">
      ...
      <section className="py-16 px-6 bg-cream">
        <MotionWrapper className="max-w-6xl mx-auto">
          <h3 className="font-serif text-2xl text-shield mb-8 text-center">Ways to Explore</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div className="w-12 h-12 rounded-lg bg-shield/10 flex items-center justify-center mb-4 group-hover:bg-shield/20 transition-colors">
                <svg className="w-6 h-6 text-shield" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-gray-600 text-sm">Explore the full family network with interactive pan, zoom, and filtering.</p>
            </Link>
            ...
            >
              ...
              <h4 className="font-semibold text-gray-900 mb-1">Interactive Globe</h4>
              <p className="text-gray-600 text-sm">See where your ancestors lived and their migration paths.</p>
            </Link>
            ...
            >
              ...
              <h4 className="font-semibold text-gray-900 mb-1">Data Explorer</h4>
              <p className="text-gray-600 text-sm">Browse the family tree by surname, birth decade, country, and more.</p>
            </Link>
            ...
            >
              ...
              <p className="text-gray-600 text-sm">Walk through history decade by decade with world events for context.</p>
            </Link>
            ...
            >
              ...
              <p className="text-gray-600 text-sm">Themed groupings: heritage, military service, immigration, and more.</p>
            </Link>
            ...
            >
              ...
              <h4 className="font-semibold text-gray-900 mb-1 flex items-center gap-2">
                Search & AI
                <kbd className="px-1.5 py-0.5 text-[10px] bg-gray-100 rounded text-gray-500">⌘K</kbd>
              </h4>
              <p className="text-gray-600 text-sm">Find ancestors by name or ask AI questions about your family history.</p>
            </button>
          </div>
      ...
      <Footer stats={treeStats && treeStats.earliestBirth != null && treeStats.latestBirth != null ? {
